...
In the output .gtf file, it will contains all representative transcripts, with attributes `"depth"` indicating the read count of the transcrpts and `"polya_flag"` listing the poly-A annotation generated by nanopolish. If users wish to merge the transcripts from multiple samples and generate the read counts, the `NIAP_merge_v1.1` can be used.

The long-reads can be filtered based on their mapping quality. For example, minimap2 will generate a mapping quality score in the output .bam file, whcih can be used for the filtering purpose by specifying the `-q` option.
...
```
perl NIAP_split_read2intron.pl [option]
  -bam <String> Input bam file
  -o <String> Output
