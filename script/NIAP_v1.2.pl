...
#Usage: perl NIAP_v1.2.pl [option]
...
#		-minjuncdist <Int> The minimal distance for splice junction correction (Default: 10 bp, empirical)
...
#	v1.2	2021-04	The subroutine argument handling was modified. The find_nearest subroutine was imprvoed to speed up the process. The default value of -minjuncdist was adjusted to 10.
...
print $gtf_fh "#perl NIAP_${version}.pl -bam $bam -o $out -q $mapq -minjuncdist $minjuncdist -t $thread -stp $stp -btp $btp";
...
while(<BAM>){
	...
	#Print for the previous chromosme and intiate the record for the new chromosome
		...
		merge_loci(\%loci);
		if($junc){
			$num_filtered_juncdist = correct_eej($current_chr, \%loci, \%ref_eej_array, \%ref_eej_hash, $minjuncdist, \%juncdist, $num_filtered_juncdist);
		}
...
sub print_usage{
	print "Usage: perl NIAP_${version}.pl [option]
	\t-bam <String> Input bam file
	\t-junc <String> The gtf file specifying splice junctions used for correction. An optional attribute, depth, can be included.
...
sub print_stat{
	my ($read_ref, $temp_out_stat, $temp_bam, $temp_num_filtered_juncdist) = @_;
...
sub read_ref_eej{
	my ($temp_junc, $ref_eej_array_ref, $ref_eej_hash_ref) = @_;
...
sub add_loci{
	my ($loci_ref, $temp_strand, $temp_leftmost, $temp_rightmost, $temp_eej, $temp_polya_flag) = @_;
...
sub find_nearest{
	my ($temp_query_ref, $temp_db_array_ref, $temp_db_hash_ref, $temp_strand, $temp_minjuncdist, $juncdist_ref) = @_;
	my $temp_nearest_dist = "NA";
	my @temp_nearest_splice_site = (0, 0);
	my $temp_coverage = 0;
	my $temp_num_db = @{$temp_db_array_ref};
	my $temp_cutoff = ($temp_minjuncdist ** 3 / 3) + ($temp_minjuncdist ** 2 / 2) + ($temp_minjuncdist / 6);
	...
	if($temp_num_db > $temp_cutoff){
		SEARCH:for(my $temp_dist = 0; $temp_dist < $temp_minjuncdist; $temp_dist++){
	...
	if($temp_nearest_dist <= $temp_minjuncdist){
...
sub correct_eej{
	my ($temp_chr, $loci_ref, $ref_eej_array_ref, $ref_eej_hash_ref, $temp_minjuncdist, $juncdist_ref, $temp_num_filtered_juncdist) = @_;
	foreach my $temp_strand (keys %{$loci_ref}){
		...
		foreach my $temp_loci (@{$loci_ref->{$temp_strand}}){
			...
			EEJ:foreach my $temp_eej (keys %{$temp_loci->[2]}){
				...
				foreach my $temp_splice_site (@temp_eej){
					...
					my $temp_nearest_splice_site_ref = find_nearest(\@temp_splice_site, \@{$ref_eej_array_ref->{$temp_chr}{$temp_strand}}, \%{$ref_eej_hash_ref->{$temp_chr}{$temp_strand}}, $temp_strand, $temp_minjuncdist, $juncdist_ref);
...
sub print_gtf{
	my ($temp_chr, $loci_ref, $temp_gtf_fh, $temp_enddist_fh, $temp_prefix) = @_;
