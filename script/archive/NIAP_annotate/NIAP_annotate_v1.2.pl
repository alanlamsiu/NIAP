...
#Usage: NIAP_annotate_v1.2.pl [option]
...
#	v1.2	This script is now able to make detailed annotation of the unkown transcripts, as antisense, intergenic, intronic and overlapping, partial transcripts of the annotated ones, which may be truncated, novel transcripts, including those with alternative ends and novel structures.
		...
		if($query_code =~ /0/){
			...
			if($ref_code =~ /^0*1+0*$/){
				...
				if($partial_flag == 1){
					...
					#Novel isoform with varied transcript end(s)
					$status = "novel_end";
				}
			}
			else{
